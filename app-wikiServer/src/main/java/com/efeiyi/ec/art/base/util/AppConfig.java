...
public class AppConfig {
    ...
    public static final String SQL_MESSAGE_DETAIL_GET_APP = "from Message where (targetUser.id = :userId and fromUser.id = :fromUserId) or (targetUser.id = :fromUserId and fromUser.id = :userId) and status<>'0' ORDER BY createDatetime ASC";
    ...
    public static final String GET_ARTIST_TOP_LIST2 = "SELECT aw.author_id,ou.username,ou.picture,ou.truename,((max(ab.price)-aw.invest_goal_money)/aw.invest_goal_money) AS bidding_rate " +
            ...
            " LIMIT ";
    public static final String GET_ARTIST_TOP_LIST3 = "SELECT ou.id as author_id,ou.username,ou.truename,ou.picture,SUM(b.pr)/SUM(a.invest_goal_money) AS bidding_rate "+
...
//            ",IF((select COUNT(c.artwork.id) from ArtWorkPraise c WHERE c.user.id = :currentUserId)>0,'true','false') AS isPraise " +
    ...
    //总收益
    public static final String SQL_REWARD_TOTAL="SELECT SUM(currentBalance) FROM ROIRecord WHERE user.id=:userId and status!='0'";
    //点赞
    public static final String SQL_PRAISE="SELECT artwork.id FROM ArtWorkPraise WHERE user.id=:userId and status!='0'";

    //投过的
    public static final String SQL_INVEST_ARTWORK = "FROM ArtworkInvest where creator.id = :userId and status ='1' GROUP BY artwork.id ORDER BY createDatetime";
}
